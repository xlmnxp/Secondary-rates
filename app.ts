...
function fixArray(Secondary: number, capacity: number, achievement?: number, step?: number): Universities[] {
    var Fix:Universities[] = universities;
    Fix.map((university,index)=>{
    ...
    Fix.sort((a,b) => <number>b.total - <number>a.total);
    
    universities_out = Fix;
    return Fix;
}

(document.getElementById("sex") as HTMLSelectElement).oninput = 
(document.getElementById("Specialty") as HTMLSelectElement).oninput = 
(document.getElementById("search") as HTMLInputElement).oninput = 
(document.getElementById("Secondary") as HTMLInputElement).oninput = 
(document.getElementById("capacity") as HTMLInputElement).oninput =
(document.getElementById("achievement") as HTMLInputElement).oninput =
(document.getElementById("step") as HTMLInputElement).oninput = ()=>{
    fixArray(parseFloat((document.getElementById("Secondary") as HTMLInputElement).value),
             parseFloat((document.getElementById("capacity") as HTMLInputElement).value),
             parseFloat((document.getElementById("achievement") as HTMLInputElement).value),
             parseFloat((document.getElementById("step") as HTMLInputElement).value));
}
...
var app4 = new Vue({
  ...
  computed: {
    ...
    universities():Universities[] {
        // cant fix this error :(
        return (this.universities_out as Universities[]).filter((university: Universities) => {
            var search = university.name.toLowerCase().indexOf((document.getElementById("search") as HTMLInputElement).value.toLowerCase()) > -1;
            var Specialty = university.name.toLowerCase().indexOf((document.getElementById("Specialty") as HTMLInputElement).value.toLowerCase()) > -1;
            var sex = university.name.toLowerCase().indexOf((document.getElementById("sex") as HTMLInputElement).value.toLowerCase()) > -1;
